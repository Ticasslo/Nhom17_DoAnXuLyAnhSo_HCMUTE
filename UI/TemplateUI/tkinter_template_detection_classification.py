...
# Model MediaPipe (.task) dùng chung cho TOÀN project:
# luôn đặt tại: Nhom17_DoAnXuLyAnhSo_HCMUTE/models/hand_landmarker.task
    ...
    class ConsoleRedirect:
        ...
        def write(self, message):
            ...
            while '\n' in self.buffer:
                ...
                # KHÔNG filter empty lines vì có thể là dòng trống hợp lệ
                self.message_queue.put(line)
        
        def flush(self):
            # Khi flush (print() tự động gọi sau mỗi print()), đưa phần còn lại vào queue
            # Print() trong Python LUÔN thêm newline ở cuối, nhưng có thể buffer chưa được xử lý
            # Nếu buffer không rỗng, đưa vào queue (đã được xử lý newline trong write())
            if self.buffer:
                # Strip trailing newlines vì đã được xử lý trong write()
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # Mỗi message từ queue là một dòng riêng biệt
                    # Hiển thị cả dòng trống để giữ format
                    try:
